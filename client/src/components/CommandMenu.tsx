...
import { Home, Users, Settings, LogOut, Film } from 'lucide-react';
import { getModKey, isTouch } from '../lib/platform';
import { format, parseISO } from 'date-fns';
import './CommandMenu.css';

interface MovieNightItem {
  id: number;
  groupName: string;
...
export default function CommandMenu() {
  ...
  const [movieNights, setMovieNights] = useState<MovieNightItem[]>([]);
  ...
  async function loadMovieNights() {
    try {
      const data = await api.get<{
        movieNights: MovieNightItem[];
      }>('/dashboard');
      setMovieNights((data.movieNights || []).slice(0, 9));
    } catch (err: unknown) {
      console.error('Failed to load movie nights:', err);
    }
  }
      ...
      <div className="command-wrapper">
        ...
        <Command.List className="command-list">
          ...
          <Command.Group heading="Navigation" className="command-group">
            <Command.Item onSelect={() => runCommand(() => navigate('/'))} className="command-item">
          ...
          <Command.Group heading="Account" className="command-group">
            <Command.Item onSelect={() => runCommand(logout)} className="command-item">
