...
interface ServiceStatus {
  overseerr: ServiceStatusItem | null;
  tautulli: ServiceStatusItem | null;
}
...
export default function Layout({ children }: LayoutProps) {
  ...
  const [stats, setStats] = useState<Stats>({ groups: 0, movieNights: 0, users: 0 });
  ...
  async function loadServiceStatus() {
    try {
      const data = await api.get<ServiceStatus>('/settings/services/status');
  ...
  async function handleRetry(service: 'overseerr' | 'tautulli') {
    setRetrying((r) => ({ ...r, [service]: true }));
    try {
      const result = await api.post<{ success: boolean }>(`/settings/retry/${service}`);
      ...
      >
        <div className="flex flex-col h-full">
                  ...
                  {serviceStatus.overseerr?.failed && (
                    <div className="flex items-center gap-2 px-3 py-2 bg-yellow-500/10 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {serviceStatus.overseerr.remainingMinutes && (
                          <p className="text-[10px] text-gray-500">
                            {serviceStatus.overseerr.remainingMinutes}m until retry
                          </p>
                        )}
                      </div>
                      ...
                      <Tooltip content="Retry" position="top">
                        ...
                        >
                          <RefreshCw
                            className={clsx('h-3.5 w-3.5', retrying.overseerr && 'animate-spin')}
                          />
                        </button>
                      </Tooltip>
                    </div>
                    ...
                    <div className="flex items-center gap-2 px-3 py-2 bg-yellow-500/10 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {serviceStatus.tautulli.remainingMinutes && (
                          <p className="text-[10px] text-gray-500">
                            {serviceStatus.tautulli.remainingMinutes}m until retry
                          </p>
                        )}
                      </div>
                      <Tooltip content="Settings" position="top">
                      ...
                      <Tooltip content="Retry" position="top">
                        ...
                        >
                          <RefreshCw
                            className={clsx('h-3.5 w-3.5', retrying.tautulli && 'animate-spin')}
                          />
                        </button>
                      </Tooltip>
                    </div>
          ...
          <div className="px-4 py-4 border-t border-gray-700">
            ...
            {version && <p className="px-3 mt-1 text-[10px] text-gray-500">v{version}</p>}
          </div>
        </div>
      </aside>
