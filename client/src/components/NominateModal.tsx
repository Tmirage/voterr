...
export default function NominateModal({
  ...
  const [movieSource, setMovieSource] = useState<'plex' | 'overseerr' | 'tmdb'>('plex');
  ...
  async function checkExternalSources() {
    try {
      const [overseerrStatus, tmdbStatus] = await Promise.all([
        api
          .get<{ configured: boolean }>('/movies/overseerr/status')
          .catch(() => ({ configured: false })),
        api
          .get<{ configured: boolean; valid: boolean; error?: string }>('/movies/tmdb/status')
          .catch(() => ({ configured: false, valid: false, error: undefined })),
      ]);
  ...
  async function loadLibrary() {
    ...
    try {
      const data =
        await api.get<Array<{ id: number; title: string; year: number; posterUrl: string }>>(
          '/movies/library'
        );
      setLibrary(data);
    } catch (err: unknown) {
  ...
  async function handleSearch(query: string, source: 'plex' | 'overseerr' | 'tmdb' = movieSource) {
    if (query.length < 2) {
      setSearchResults([]);
      return;
    }
    ...
    try {
      ...
      const data = await api.get<
        ...
        | { results: Array<{ id: number; title: string; year: number; posterUrl: string }> }
      ...
      if (source === 'overseerr' && results.length === 0 && query.length >= 3) {
        ...
        try {
          const status = await api.get<{ overseerr?: { failed: boolean } }>(
        ...
        <div className="p-3 sm:p-4 border-b border-gray-700 space-y-2 sm:space-y-3">
          ...
          {(overseerrConfigured || tmdbConfigured) && (
            <div className="flex gap-2">
              <button
                onClick={() => {
                  setMovieSource('plex');
                  if (searchQuery.length >= 2) handleSearch(searchQuery, 'plex');
                }}
                className={clsx(
              ...
              {overseerrConfigured && (
                <button
                  onClick={() => {
                    setMovieSource('overseerr');
                    if (searchQuery.length >= 2) handleSearch(searchQuery, 'overseerr');
                  }}
                  className={clsx(
                    'px-3 py-1.5 rounded-lg text-sm transition-colors',
                    movieSource === 'overseerr'
                      ? 'bg-indigo-600 text-white'
                      : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                  )}
                >
              ...
              {tmdbConfigured && !overseerrConfigured && (
                <button
                  onClick={() => {
                    setMovieSource('tmdb');
                    if (searchQuery.length >= 2) handleSearch(searchQuery, 'tmdb');
                  }}
                  className={clsx(
                    'px-3 py-1.5 rounded-lg text-sm transition-colors',
                    movieSource === 'tmdb'
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={
                movieSource === 'plex' ? 'Search your Plex library...' : 'Search all movies...'
              }
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {alreadyNominated && (
                        <p className="text-xs text-indigo-400 mt-1">Already nominated</p>
                      )}
                    </div>
                  </button>
                );
