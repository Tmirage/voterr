...
interface Group {
  ...
  memberCount: number;
  upcomingNight?: { date: string } | null;
}
...
export default function Groups() {
  ...
  async function loadGroups() {
    try {
      const data =
        await api.get<
          Array<{ id: number; name: string; imageUrl: string | null; memberCount: number }>
        >('/groups');
      setGroups(data);
    } catch (err: unknown) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    try {
      const group = await api.post<{ id: number; name: string; imageUrl: string | null }>(
        '/groups',
        newGroup
      );
                ...
                <div className="flex items-center gap-4 mt-3 text-sm text-gray-500">
                  ...
                  {group.upcomingNight && (
                    <span className="flex items-center gap-1 text-indigo-400">
                      ...
                      {new Date(group.upcomingNight.date).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                      })}
                    </span>
