...
import { UserPlus, Trash2, Download, Shield, ShieldOff, Pencil, Check, X } from 'lucide-react';
...
export default function Users() {
  ...
  const [plexFriends, setPlexFriends] = useState<PlexFriend[]>([]);
  ...
  async function loadPlexFriends() {
    try {
      const friends =
        await api.get<Array<{ id: string; username: string; email: string; thumb: string }>>(
          '/users/plex-friends'
        );
      setPlexFriends(friends);
      setShowImportPlex(true);
    } catch (err: unknown) {
      console.error('Failed to load Plex friends:', err);
    }
  }

  async function handleImportPlex(friends: PlexFriend[]) {
    try {
  ...
  function toggleFriendSelection(friend: PlexFriend) {
  ...
  function startEditUser(u: UserData) {
    setEditingUser(u);
        ...
        users.map((u) =>
          u.id === editingUser.id
            ? {
                ...
                email: editingUser.isLocal ? editForm.email?.trim() || null : u.email,
              }
            : u
        )
      );
      ...
      <details className="bg-gray-800 rounded-xl mb-6 group">
        ...
        <div className="px-4 pb-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <div className="flex items-start gap-2">
            <span className="px-1.5 py-0.5 rounded bg-purple-600/30 text-purple-300 shrink-0">
              App Admin
            </span>
            <span className="text-gray-400">Full control over settings and users.</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="px-1.5 py-0.5 rounded bg-indigo-600/30 text-indigo-300 shrink-0">
              Admin
            </span>
            <span className="text-gray-400">Can manage groups and movie nights.</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="px-1.5 py-0.5 rounded bg-orange-600/30 text-orange-300 shrink-0">
              Plex
            </span>
            <span className="text-gray-400">Login with Plex account.</span>
          </div>
          <div className="flex items-start gap-2">
            <span className="px-1.5 py-0.5 rounded bg-gray-600/50 text-gray-400 shrink-0">
              Local
            </span>
      ...
      <div className="bg-gray-800 rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-gray-700">
          {users.map((u) => (
            <div
              key={u.id}
              className="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2"
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
                    ...
                    {u.isAppAdmin && (
                      <span className="px-1.5 py-0.5 text-xs rounded bg-purple-600/30 text-purple-300">
                        App Admin
                      </span>
                    )}
                    {u.isAdmin && !u.isAppAdmin && (
                      <span className="px-1.5 py-0.5 text-xs rounded bg-indigo-600/30 text-indigo-300">
                        Admin
                      </span>
                    )}
                  </div>
            ...
            >
              ...
              <div className="flex gap-3 justify-end items-center">
                ...
                >
                  Cancel
                  {!isTouch() && (
                    <kbd className="text-[10px] px-1.5 py-0.5 bg-gray-700 rounded">Esc</kbd>
                  )}
                </button>
                <button
                  type="submit"
            ...
            <form onSubmit={handleCreateLocal} className="space-y-4">
              ...
              <div className="flex gap-3 justify-end items-center">
                ...
                >
                  Create
                  {!isTouch() && (
                    <kbd className="text-[10px] px-1.5 py-0.5 bg-black/20 rounded">
                      {getModKey()}↵
                    </kbd>
                  )}
                </button>
