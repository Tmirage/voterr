...
const STEPS = [
  { id: 'plex', title: 'Plex Account', description: 'Connect your Plex media server' },
  {
    id: 'overseerr',
    title: 'Movie Search',
    description: 'Optional: Search movies beyond your Plex library',
  },
  {
    id: 'tautulli',
    title: 'Watch History',
    description: 'Optional: Track who has seen which movies',
  },
];
...
export default function Setup() {
  ...
  const [plexUser, setPlexUser] = useState<PlexUserInfo | null>(null);
  ...
  const [testResult, setTestResult] = useState<{
    overseerr: TestResult | null;
    tautulli: TestResult | null;
  }>({ overseerr: null, tautulli: null });
      ...
      try {
        ...
        if (authToken) {
          ...
          const result = await api.post<{
            user: { username: string; email: string; thumb: string };
          }>('/setup/plex-auth', { authToken });
  ...
  async function testTautulli() {
    ...
    try {
      ...
      setTestResult((r) => ({
        ...r,
        tautulli: { success: false, message: e instanceof Error ? e.message : 'Connection failed' },
      }));
    } finally {
            ...
            <div>
              ...
              <div className="mt-4 p-3 bg-gray-700/50 rounded-lg">
                <p className="text-sm text-gray-300 mb-2">
                  Your Plex library is the primary movie source:
                </p>
                ...
                {testResult.overseerr && (
                  <span
                    className={clsx(
                      ...
                      testResult.overseerr.success ? 'text-green-400' : 'text-red-400'
                    )}
                  >
                    {testResult.overseerr.success ? (
              ...
              <div className="flex items-center gap-3 mt-4">
                ...
                {testResult.tautulli && (
                  <span
                    className={clsx(
                      'flex items-center gap-1 text-sm',
                      testResult.tautulli.success ? 'text-green-400' : 'text-red-400'
                    )}
                  >
                    {testResult.tautulli.success ? (
                      <Check className="h-4 w-4" />
                    ) : (
                      <X className="h-4 w-4" />
                    )}
