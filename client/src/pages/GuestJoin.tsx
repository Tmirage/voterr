...
export default function GuestJoin() {
  ...
  const [pinGroupInfo, setPinGroupInfo] = useState<PinGroupInfo | null>(null);
  ...
  const validateInvite = useCallback(
    async (pinCode: string | null = null) => {
      try {
        setLoading(true);
        const url = pinCode
        ...
        const data = await api.get<
          { requiresPin: true; groupName: string } | (InviteData & { requiresPin?: false })
        >(url);

        if (data.requiresPin) {
          setRequiresPin(true);
          setPinGroupInfo({ groupName: data.groupName });
          setLoading(false);
          return;
        }
  ...
  if (error) {
    const errorMessages = {
      'Invalid invite link':
        'This invite link is no longer valid. It may have been regenerated or deleted.',
    ...
    const friendlyMessage =
      errorMessages[error] ||
      'This invite link is not valid. Please check the link or ask for a new one.';
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="bg-gray-800 rounded-xl p-8 max-w-md w-full">
        ...
        {invite.isCancelled ? (
          ...
          invite.topNominations &&
          invite.topNominations.length > 0 && (
            <div className="bg-gray-700 rounded-xl p-4 mb-6">
