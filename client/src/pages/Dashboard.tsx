...
interface DashboardNomination {
  ...
  mediaType?: string;
  isBlocked?: boolean;
  watchedBy?: Array<{ userId: number; username: string }>;
  blockedBy?: Array<{ userId: number; username: string }>;
  [key: string]: unknown;
}

interface DashboardMovieNight {
  id: number;
  groupId: number;
  groupName: string;
  ...
  nominations: DashboardNomination[];
  userRemainingVotes: number;
  maxVotesPerUser: number;
  winningMovieId: number | null;
  winningMovieTitle: string | null;
  winner: DashboardNomination | null;
  members: Array<{
    id: number;
    username: string;
  ...
  memberVotingStatus: Array<{ id: number; hasVoted: boolean }>;
  attendance: Array<{ userId: number; status: 'attending' | 'absent' | null }>;
...
interface DashboardData {
  ...
  movieNights: DashboardMovieNight[];
  plexServerId?: string;
}
...
export default function Dashboard() {
  ...
  const [nominateNightId, setNominateNightId] = useState<number | null>(null);
  const [showInvite, setShowInvite] = useState(false);
  const [, setInviteNightId] = useState<number | null>(null);
  const [inviteUrl, setInviteUrl] = useState('');
  const [inviteId, setInviteId] = useState<number | null>(null);
  const [showHostPicker, setShowHostPicker] = useState(false);
  const [hostPickerNightId, setHostPickerNightId] = useState<number | null>(null);
  ...
  function getSortedNominations(night: DashboardMovieNight): DashboardNomination[] {
    ...
    return sorted.length
      ? sorted
      : [...(night.nominations || [])].sort((a, b) => b.voteCount - a.voteCount);
  }

  function getVotingData(night: DashboardMovieNight) {
  ...
  async function handleSetAttendance(nightId: number, status: string) {
    try {
  ...
  function handleVote(nominationId: number, nightId: number) {
    voting.vote(nominationId, nightId);
  }

  function handleUnvote(nominationId: number, nightId: number) {
    voting.unvote(nominationId, nightId);
  }

  async function handleUndecide(nightId: number) {
    try {
      await api.post(`/votes/movie-night/${nightId}/undecide`);
      await loadData();
    } catch (err: unknown) {
      console.error('Undecide failed:', err);
    }
  }

  async function handleDecide(nightId: number, nominationId: number | null = null) {
    try {
  ...
  async function handleCreateInvite(nightId: number) {
    try {
  ...
  async function handleSetHost(nightId: number, userId: number | null) {
    try {
  ...
  function getUserAttendance(night: DashboardMovieNight) {
      ...
      {movieNights.length === 0 ? (
        <div className="bg-gray-800 rounded-xl p-12 text-center">
          ...
          <p className="text-gray-500 text-sm mt-1">
            {groups.length > 0
              ? 'Schedule a movie night in one of your groups'
              : 'Create a group and schedule to get started'}
          </p>
          {groups.length > 0 ? (
            <Link
        ...
        <div className="space-y-6">
          {movieNights.map((night) => {
            ...
            const currentNominations =
              night.nominations?.filter((n) => n.id !== night.winningMovieId) || [];
            ...
            return (
              <div key={night.id} className="bg-gray-800 rounded-xl overflow-hidden">
                <div className="p-4 md:p-6 border-b border-gray-700">
                  ...
                  {night.members && night.members.length > 0 && (
                    <div className="mt-4">
                      <MemberStatusList
                        ...
                        onSetAttendance={async (userId, status) => {
                          try {
                            await api.post(
                              `/schedules/movie-nights/${night.id}/attendance/${userId}`,
                              { status }
                            );
                            await loadData(true);
                          } catch (err: unknown) {
                  ...
                  <div className="p-4 md:p-6">
                    ...
                    {currentNominations.length === 0 ? (
                      ...
                      <AnimatedList className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                        {sortedNominations.map((nomination, _index) => {
                          return (
                            <div
                              key={nomination.id}
                              className={clsx(
                              ...
                              <div className="mt-2 flex-1 min-w-0">
                                ...
                                {nomination.watchedBy && nomination.watchedBy.length > 0 && (
                                  <p className="text-[10px] text-gray-500 mt-1">
                                    Watched by{' '}
                                    {nomination.watchedBy.map((w) => w.username).join(', ')}
                                  </p>
                                )}
                              </div>
                                    ...
                                    nomination.id !== night.winningMovieId && (
                                      <Tooltip content="Pick as winner">
                                        <button
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            handleDecide(night.id, nomination.id);
                                          }}
