...
import { Server, Check, X, Loader2, Film, HardDrive, Trash2, Tv, Search, Eye } from 'lucide-react';
...
export default function Settings() {
  ...
  const [plexInfo, setPlexInfo] = useState<{
    serverName: string;
  ...
  const [testResults, setTestResults] = useState<{
    overseerr: { success: boolean; message?: string } | null;
    tautulli: { success: boolean; message?: string } | null;
    tmdb: { success: boolean; message?: string } | null;
  }>({
    overseerr: null,
    tautulli: null,
    tmdb: null,
  ...
  async function handleResetOverseerr() {
    ...
    try {
      ...
      console.error('Failed to reset Overseerr:', err);
    } finally {
  ...
  async function handleResetTautulli() {
    ...
    try {
      ...
      console.error('Failed to reset Tautulli:', err);
    } finally {
  ...
  async function testOverseerr() {
    ...
    setTestResults((r) => ({ ...r, overseerr: null }));
    try {
      await api.post('/settings/test/overseerr', {
        url: config.overseerrUrl,
        apiKey: config.overseerrApiKey,
      });
      setTestResults((r) => ({
        ...r,
        overseerr: { success: true, message: 'Connected successfully' },
      }));
      notifyServiceStatusChanged();
    } catch (err: unknown) {
      setTestResults((r) => ({
  ...
  async function testTautulli() {
    ...
    setTestResults((r) => ({ ...r, tautulli: null }));
    try {
      await api.post('/settings/test/tautulli', {
        url: config.tautulliUrl,
        apiKey: config.tautulliApiKey,
      });
      setTestResults((r) => ({
        ...r,
        tautulli: { success: true, message: 'Connected successfully' },
      }));
      notifyServiceStatusChanged();
    } catch (err: unknown) {
      setTestResults((r) => ({
  ...
  async function loadCacheStats() {
    try {
      const stats = await api.get<{ count: number; size: number; sizeMB?: number }>(
  ...
  async function loadPlexInfo() {
    try {
      ...
      console.error('Failed to load Plex info:', err);
    }
  }
  ...
  async function testTmdb() {
    ...
    try {
      ...
      setTestResults((r) => ({
        ...r,
        tmdb: { success: false, message: err instanceof Error ? err.message : 'Invalid API key' },
      }));
    } finally {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-gray-800 rounded-xl p-6">
        ...
        <p className="mt-3 text-xs text-gray-500">
          Plex is your primary movie source. Movies from your Plex library are always available for
          nomination.
        </p>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              Search and nominate movies not in your Plex library
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-700/30 rounded-lg p-4 mb-4">
          ...
          <ul className="text-xs text-gray-400 space-y-1">
            ...
            <li>
              - Configure at least one (Overseerr or TMDB API key) to search beyond your Plex
              library
            </li>
          </ul>
        </div>
        ...
        <div className="border-t border-gray-700 pt-4 mt-4">
          ...
          <p className="text-xs text-gray-500 mb-3">
            Overseerr provides movie search with request management. If you have Overseerr, use this
            instead of a direct TMDB key.
          </p>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-4 py-2 bg-gray-700 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center gap-2"
              >
                {testing.tmdb ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Test API Key'}
              </button>
              {testResults.tmdb && (
                <div
                  className={clsx(
      ...
      <div className="bg-gray-800 rounded-xl p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              Cache Plex movie images locally to reduce bandwidth
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400">
                Store movie posters locally instead of fetching from Plex
              </p>
            </div>
            <button
